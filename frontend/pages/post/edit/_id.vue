			...
			<div class="flex justify-center px-6 my-12">
				...
				<div class="w-full xl:w-3/4 lg:w-11/12 flex">
					...
					<div class="w-full lg:w-7/12 bg-white p-5 rounded-lg lg:rounded-l-none">
						<h3 class="pt-4 text-2xl text-center">Update a Post!</h3>
						<form class="px-8 pt-6 pb-8 mb-4 bg-white rounded"  ref="addpostform" @submit.prevent="addPost" enctype="multipart/form-data">
							...
							<div class="mb-4 md:flex md:justify-between">
								...
								<input
									...
									required
								/>
								<img height="100" width="100" :src="`${post.image_link}`" alt="featured image" />
							</div>
  ...
  methods: {
    addPost() {
      try {
			...
			this.$axios.post("/api/post/update/"+this.$route.params.id, formData).then((res) => {
          	this.$toast.success('Successfully Post Updated',{
				onComplete : function(){
					this.$router.push({
						path: "/",
					});
